...
namespace API.Controllers
{
    ...
    public class UsersController : BaseApiController
    {
        ...
        {
            var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername()); // It gets us the username token that the API uses to auth. the users -> This is the user we will be updating; Then gets the user by its username

            _mapper.Map(memberUpdateDto, user); // Automatically update properties in user;  example --> (user.City = memberUpdateDto.City) 

            _unitOfWork.UserRepository.Update(user); //This guarantees to not cause exceptions, even if we update any propertiees or not
